...
public record AddEntityAfterImageWithTextureParticle(int id) implements BasePacket {
    ...
    public void execute(Player player) {
        if(Minecraft.getInstance().player != null && Minecraft.getInstance().level != null){
           ...
           if(entity != null){
               Minecraft.getInstance().level.addParticle(WuKongParticles.ENTITY_AFTER_IMAGE_WITH_TEXTURE.get(), entity.getX(), entity.getY(), entity.getZ(), Double.longBitsToDouble(entity.getId()), 0.0, 0.0);
           }
        }
    }
}

 ...
 * 共用冷却
 */
public abstract class QiShuSkill extends CoolDownSkill {
    public QiShuSkill(Builder<? extends Skill> builder) {
        super(builder);
    }
    ...
    public void executeOnServer(ServerPlayerPatch executer, FriendlyByteBuf args) {
        ...
        wkPlayer.setCooldownQiShu(getMaxCooldown());
    }
    ...
    public void updateContainer(SkillContainer container) {
        ...
        if(!container.getExecuter().isLogicalClient()){
            ...
            wkPlayer.setCooldownQiShu(Math.max(wkPlayer.getCooldownQiShu() - 1, 0));
            manager.setDataSync(COOLDOWN_TIMER, wkPlayer.getCooldownQiShu(), serverPlayer);
        }
    }
}

...
public class EntityAfterImageWithTextureParticle extends CustomModelParticle<AnimatedMesh> {
    ...
    public EntityAfterImageWithTextureParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, AnimatedMesh particleMesh, OpenMatrix4f[] matrices, Matrix4f modelMatrix, LivingEntityPatch<?> entityPatch) {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTicks) {
        ...
        this.particleMesh.drawModelWithPose(poseStack, vertexConsumer, this.getLightColor(partialTicks), this.rCol, this.gCol, this.bCol, alpha, OverlayTexture.NO_OVERLAY, this.entityPatch.getArmature(), this.poseMatrices);
    }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ...
            if (entitypatch != null && ClientEngine.getInstance().renderEngine.hasRendererFor(entitypatch.getOriginal())) {
                ...
                EntityAfterImageWithTextureParticle particle = new EntityAfterImageWithTextureParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, mesh, matrices, poseStack.last().pose(), entitypatch);
                return particle;
            } else {
                return null;
            }
